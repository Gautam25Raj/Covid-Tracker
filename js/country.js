...
allCountryList.sort(compare);

let numOfUlList = 3;

const searchCountry = document.querySelector('.search-country');
const countryList = document.querySelector('.country-list');
const changeCountryBtn = document.querySelector('.country-change');
const closeListBtn = document.querySelector('.close');
const input = document.getElementById('search-input');

function createCountryList() {
  const numberOfCountries = allCountryList.length;

  let count = 0,
    ulList;

  allCountryList.forEach((country, index) => {
    if (index % Math.ceil(numberOfCountries / numOfUlList) == 0) {
      ulList = `list-${count}`;
      countryList.innerHTML += `<ul id='${ulList}'></ul>`;
      count++;
    }
